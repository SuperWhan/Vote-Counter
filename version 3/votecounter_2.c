...
int read_write_Agg(struct dnode dl, struct dnode tid)
{
  char* in_path = malloc(1024);
  char* out_path = malloc(1024);
  char* temp = malloc(256);
  ...
  strcpy(in_path, tid.path);
  strcat(in_path, "/");
  strcat(in_path, tid.name);
  strcat(in_path, ".txt");
  ...
  size_t len = 0;
  ssize_t read;
  if(access(out_path, F_OK) != -1) // file exist
  {
    output = fopen(out_path, "r");
...
void* read_write_thread(void *arg)
{
  ...
  int* children = leaves_finder(three_args->dl, three_args->lc);
  char* logfile = malloc(1024);
  int tid = pthread_self();
  int j=0;
  struct dnode t_node;
  t_node = three_args->dn;
  FILE* output;
  ...
  {
    pthread_mutex_lock(three_args->mutex);
    read_write_Agg(three_args->dl[three_args->dn.parent], three_args->tc);
